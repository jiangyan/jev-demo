...
A demonstration of [Jev](https://typesafe.ai/blog/introducing-system-one-models-and-jev),
TypeSafe AI's System One model, built around the thing that makes it different from
an LLM: it returns **typed answers with calibrated probabilities**, and it returns
them fast enough to sit in the hot path.

A queue of support tickets goes in. Each one gets a single call to Jev that answers
six questions at once. The software then decides, from the confidence it gets back,
whether to act unattended, ask a person to confirm, or hand the ticket over. You can
move the thresholds and watch the whole queue re-decide, and there is a panel that
checks whether the confidence number deserves the trust the thresholds place in it.
...
There is a terminal version of the same thing:
...
**The fixtures in this repository are synthetic.** They were generated locally by
`src/core/synthetic.ts`, not recorded from Jev, because the machine this was built on
could not reach `api.typesafe.ai`. They are shaped exactly like real responses, which
is enough to exercise every part of the demo offline, and they tell you **nothing**
about how well the real model performs.

The generator is deliberately overconfident by a known amount. That is on purpose: it
gives the calibration panel a real fault to detect, so you can see that the harness
works. Finding a fault we planted ourselves is a test of the measuring instrument, not
a measurement of Jev. Run `npm run record` with a key and the numbers start meaning
something.
...
```
src/core/questions.ts     the decision sheet
